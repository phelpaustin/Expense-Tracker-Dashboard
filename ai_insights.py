...
from config import CLAUDE_MODEL, Columns
...
def _summarise(df: pd.DataFrame) -> Dict:
    ...
    monthly = df2.groupby(Columns.YEAR_MONTH)["PricePaid"].sum().sort_index()
    ...
    if "Category" in df2.columns:
        ...
        this = df2[df2[Columns.YEAR_MONTH] == curr_m]
        if not this.empty:
            summary["by_category_this_month"] = top_n(
                this.groupby("Category")["PricePaid"].sum()
            )
        prev = df2[df2[Columns.YEAR_MONTH] == prev_m]
        if not prev.empty:
...
def _render_report(cached: Tuple[str, str]) -> None:
    ...
    body = (
        f"<ul style='margin:0;padding-left:1.2rem;line-height:1.75;font-size:0.92rem;'>"
        f"{items_html}</ul>"
    )
    ...
    render_ai_card(
        body, provider,
        cache_key=f"ai_report_{pd.Timestamp.now().strftime('%Y-%m')}",
        refresh_key="refresh_report_btn",
        accent=t_colors.get("accent", "#6366f1"),
        bg=t_colors.get("paper", "#fff"),
        border=t_colors.get("border", "#e2e8f0"),
        fg=t_colors.get("fg", "#0f172a"),
        muted=t_colors.get("muted", "#64748b"),
        with_refresh=False,
    )

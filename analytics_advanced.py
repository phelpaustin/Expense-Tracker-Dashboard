...
def spending_insights(df: pd.DataFrame):
    ...
    # ── AI-enhanced narrative insights ────────────────────────────────────────
    st.markdown("---")
    _ai_spending_narrative(df2)


def _ai_spending_narrative(df2: pd.DataFrame) -> None:
    """Call AI to narrate spending patterns in plain English."""
    import json
    try:
    ...
    cache_key = f"ai_analytics_narrative_{pd.Timestamp.now().strftime('%Y-%m')}"
    if cache_key not in st.session_state:
        if not st.button("🤖 Generate AI Narrative", key="ai_analytics_btn"):
            return

        monthly = df2.groupby(Columns.YEAR_MONTH)[Columns.PRICE_PAID].sum().tail(6)
        cat_totals = (df2.groupby(Columns.CATEGORY)[Columns.PRICE_PAID].sum()
                      .nlargest(8).round(2).to_dict()
                      if Columns.CATEGORY in df2.columns else {})
        dow = (df2.groupby("DayOfWeek")[Columns.PRICE_PAID].sum().round(2).to_dict()
               if "DayOfWeek" in df2.columns else {})

        payload = {
            "monthly_totals_last6": {str(k): round(float(v), 2) for k, v in monthly.items()},
        ...
        system = (
            ...
            "Write a 4-5 sentence plain-English narrative about the user's spending trends. "
            "Be specific with numbers, identify the most interesting pattern, "
            "and end with one actionable suggestion. No bullet points — flowing prose only."
        )
        user = (
            f"Here is my spending analytics summary:\n{json.dumps(payload, indent=2)}\n\n"
            "Write a short narrative analysis of my spending patterns."
        )
        with st.spinner("AI is analysing your patterns…"):
    ...
    st.markdown(
        ...
        f"padding:1rem 1.3rem;font-size:0.92rem;line-height:1.7;color:#0f172a;'>"
        f"{text}"
        f"<div style='margin-top:0.5rem;font-size:0.7rem;color:#94a3b8;'>"
        f"Generated by {provider}</div></div>",
        unsafe_allow_html=True,
    )
    if st.button("🔄 Refresh", key="refresh_analytics_ai"):

...
def get_ai_alert_thresholds(df: pd.DataFrame, budgets: dict) -> dict:
    ...
    for cat, budget in cat_budgets.items():
        monthly = df2[df2[Columns.CATEGORY] == cat].groupby(Columns.YEAR_MONTH)[Columns.PRICE_PAID].sum()
        if monthly.empty:
            continue
...
def ai_alert_summary(df: pd.DataFrame, budgets: dict) -> None:
    ...
    render_ai_card(
        body, provider, cache_key=cache_key, refresh_key="refresh_alert_ai",
        accent="#ef4444", bg=t_bg, border=t_border, fg=t_fg, muted=t_muted,
        with_refresh=False,
    )

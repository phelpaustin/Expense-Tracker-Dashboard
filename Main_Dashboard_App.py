...
from alerts_engine import get_all_alerts, get_enabled_alerts, generate_daily_summary
...
def show_alerts_banner(df):
    ...
    # Use get_enabled_alerts so user alert preferences are respected
    try:
...
try:
    from budget_manager import budget_dashboard_ui, budget_setup_ui, load_budgets
    HAS_BUDGET = True
except ImportError:
    HAS_BUDGET = False
    def load_budgets():
...
# ── NEW: Price Tracker & Financial Metrics ────────────────────────────────────
try:
...
def build_sidebar(t: dict):
    ...
    if HAS_PRICE_TRACKER:    pages["💰 Price Tracker"]      = "price_tracker"
    if HAS_FINANCIAL_METRICS: pages["📈 Financial Metrics"] = "financial_metrics"
...
def page_dashboard():
    ...
    # ── Active budget alerts banner ──
    if not df.empty:
        show_alerts_banner(df)
...
def page_analytics():
    ...
    st.caption(f"📊 **{len(df_filtered):,} transactions** from {start_date} to {end_date}{category_text} | Viewing by: {granularity}")
    ...
    with st.expander("💾 **EXPORT FILTERED DATA**", expanded=False):
        ...
        with col_exp1:
            ...
            st.download_button("📄 Download CSV", csv, filename, "text/csv", width="stretch")
        with col_exp2:
            try:
...
elif page == "price_tracker" and HAS_PRICE_TRACKER:
    price_tracker_ui(df)
elif page == "financial_metrics" and HAS_FINANCIAL_METRICS:
